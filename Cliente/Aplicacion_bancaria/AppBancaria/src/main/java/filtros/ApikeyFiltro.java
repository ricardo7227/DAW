        ...
        try {
            ...
            if (apikeyDB != null) {
                if (!servicios.checkLimitApikey(apikeyDB)) {
                    ...
                    request.setAttribute(Constantes.JSON, new GenericResponse(HttpStatus.SC_FORBIDDEN, Mensajes.SUPERADO_LIMITES_PETICIONES));
                }

            } else {
                ((HttpServletResponse) response).setStatus(HttpStatus.SC_UNAUTHORIZED);
                request.setAttribute(Constantes.JSON, new GenericResponse(HttpStatus.SC_UNAUTHORIZED, Mensajes.FALTA_API_KEY));
            }
