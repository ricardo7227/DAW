...
public class UsersDAO {
    ...
    /**
     * Busca un registro por nombre, si existe devuelve el objeto LOgin
     *
     * @param usuario
     * @return usuario
     */
    public Login getLoginUserJDBCTemplate(Login usuario) {

        JdbcTemplate jtm = new JdbcTemplate(
                DBConnection.getInstance().getDataSource());
        Login user = null;

        String resultadoQuery = jtm.query(SqlQuery.SELECT_NOMBRE, new Object[]{usuario.getNombre(),usuario.getPassword()}, new ResultSetExtractor<String>() {
            @Override
        ...
        if (resultadoQuery != null) {
            user = (Login) jtm.queryForObject(SqlQuery.SELECT_NOMBRE, new Object[]{usuario.getNombre()},//funciona siempre que exista un dato en la base de datos
    ...
    public User getUserJDBCTemplate(Login usuario) {

        JdbcTemplate jtm = new JdbcTemplate(
                DBConnection.getInstance().getDataSource());
        User user = null;

        user = (User) jtm.queryForObject(SqlQuery.SELECT_NOMBRE, new Object[]{usuario.getNombre()},
                new BeanPropertyRowMapper(User.class));

        return user;
    }

    public int updateActivoByNombreJDBCTemplate(User usuario) {

        JdbcTemplate jtm = new JdbcTemplate(
                DBConnection.getInstance().getDataSource());

        int rowsAffected = jtm.update(SqlQuery.UPDATE_ACTIVO_USER, usuario.isActivo(), usuario.getNombre());
    ...
    public User insertIntoUserJDBCTemplate(User usuario) {
