...
<html>
	<head>
		...
		<script>
			function pregunta()
			{
				var seleccionado= document.getElementById('seleccionado').checked;

				if (seleccionado==true)
				{
					if (confirm('¿Desea borrar los datos definitivamente?'))
						{
				...
				{
				alert("No se ha seleccionado ninguna ");
				}	
			}
			...
			$(document).ready(function()
				...
				$("#Buscar").click(function(evento)
					{	
						...
						$("#destino").load("cierre3.php",{cajanumcu:document.getElementById('cajanumcu').value},function()
							{
	...
	<body>	
		...
		<div class="w3-animate-opacity formulario w3-card-16">
			<form method="post" action="cierre2.php" name="formularioborrar" class="w3-container">
				<div class="w3-half">
					<p> 
						<label class="w3-label w3-text-teal"><b>Numero de Cuenta:</b></label>
						<input id="cajanumcu" name="cajanumcu" class="w3-input w3-border w3-light-grey" type="text">
						
						<div name="cargando" style="display:none;" id="cargando" class="w3-opacity w3-animate-bottom w3-card-8">					
							<p><i class="fa fa-spinner fa-spin"></i> Cargando los resultados</p>
						</div>
						<button id="Buscar" name="Buscar" class="w3-btn w3-medium w3-teal">Buscar Numero de Cuenta</button>
					</p>
				</div>
				<div class="w3-half" id="destino">
