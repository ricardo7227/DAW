...
function compara_fechas3($fecha1,$fecha2){
	 ...
	 list($ano1,$mes1,$dia1)=split("-", $fecha_1[0]);
	 list($hora1,$min1,$seg1)=split(":", $fecha_1[1]);
	 
	 $fecha_2 = split(" ", $fecha2);
	 list($ano2,$mes2,$dia2) = split("-", $fecha_2[0]);
	 list($hora2,$min2,$seg2) = split(":", $fecha_2[1]);
	 
	 $dif = mktime($hora1, $min1, $seg1, $mes1, $dia1, $ano1) - mktime($hora2, $min2, $seg2, $mes2, $dia2, $ano2);
	 return ($dif);                         
}
...
function proxPrevDia($fecha, $tipo = 'pro', $offset = 1){
	 ...
	 switch($tipo){
	 	case 'pro':{
			$fecha2 = date("Y-m-d", mktime(0, 0, 0, $mes, ($dia+$offset), $ano));
			//$fecha2 = date($ano.'-'.$mes.'-'.$dia)+1;
		}
		break;
		case 'pre':{
			$fecha2 = date("Y-m-d", mktime(0, 0, 0, $mes, ($dia-$offset), $ano));
